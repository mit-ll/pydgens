...
def _validate_newton_solve_params(
    *,
    residual_rtol: float,
    residual_atol: float,
    max_iters: int,
    max_rejects: int,
    return_last_accepted: bool,
) -> None:
    if not isinstance(max_iters, int) or max_iters < 0:
        raise ValueError(f"max_iters must be an int >= 0, got {max_iters}")
    if not isinstance(max_rejects, int) or max_rejects < 0:
        raise ValueError(f"max_rejects must be an int >= 0, got {max_rejects}")
    if not isinstance(return_last_accepted, bool):
        raise TypeError(f"return_last_accepted must be bool, got {type(return_last_accepted)}")

    for name, v in [("residual_rtol", residual_rtol), ("residual_atol", residual_atol)]:
        if not isinstance(v, (float, int)):
            raise TypeError(f"{name} must be float-like, got {type(v)}")
        if not math.isfinite(float(v)):
            raise ValueError(f"{name} must be finite, got {v}")
...
def newton_solve_autodiff(
    ...
    *,
    ...
    max_iters: int, # = 25,
    max_rejects: int, # = 5,
    # step controls (forwarded to newton_step_autodiff)
    reg0: float, # = 0.0,
    reg1_min: float, # = 1e-12,
    reg_increase: float, # = 10.0,
    reg_max: float, # = 1e8,
...
) -> Tuple[trajtypes.FixedStepPrimalDualTrajectory, altypes.NewtonSolveDiag]:
    """
    Solve G(z)=0 using repeated Newton steps with autodiff Jacobian.

    Termination policy
    ------------------
    - Converged if ||G||_2 <= residual_tol.
    - Stop if too many rejected steps in a row (max_rejects).
    - Stop if max_iters reached.

    Parameters are forwarded to `newton_step_autodiff`.

    Returns
    -------
    op_best : FixedStepPrimalDualTrajectory
        Final trajectory. If return_last_accepted=True, this is the last accepted
        iterate (safer). Otherwise it is the final iterate (could be unchanged if last step rejected).
    diag : NewtonSolveDiag
        Iteration history and termination reason.

    Notes
    -----
    - Reference Le Cleach'h et al. "ALGames: A Fast Solver For Constrained Dynamic Games", Algorithm 2
    """

    # --- parameter validation ---
    _validate_newton_solve_params(
        residual_rtol=residual_rtol, residual_atol=residual_atol, max_iters=max_iters, max_rejects=max_rejects, return_last_accepted=return_last_accepted
    )
    _validate_reg_params(
        reg0=reg0, reg1_min=reg1_min, reg_increase=reg_increase, reg_max=reg_max, reg_max_iters=reg_max_iters
    )
    ...
    if not bool(jnp.all(jnp.isfinite(g))):
        diag = altypes.NewtonSolveDiag(
            ...
            accepted=tuple(),
            solve_ok=tuple(),
            reason="nonfinite_residual_at_start",
        )
        return op0, diag
    g_norm = residual_norm(g, kind=normkind)
    g_norms.append(g_norm)

    # check if residual has already converged
    if g_norm <= residual_atol:
        diag = altypes.NewtonSolveDiag(
            converged=True,
            iters=0,
            g_norms=tuple(g_norms),
            step_norms=tuple(),
            alphas=tuple(),
            regs=tuple(),
            accepted=tuple(),
    ...
    for k in range(max_iters):
        ...
        accepted.append(step_diag.accepted)
        solve_ok.append(step_diag.solve_ok)
        ...
        # compute residual norm at new/current op
        z = pack_decision_vars_1d(op)
        g = compute_al_residual_flat_from_decision_vars(
            nlgame, z, op, alstate,
            discretize_method=discretize_method,
        ...
        if not bool(jnp.all(jnp.isfinite(g))):
            diag = altypes.NewtonSolveDiag(
                converged=False,
                iters=k + 1,
                g_norms=tuple(g_norms) + (float("nan"),),
                step_norms=tuple(step_norms),
                alphas=tuple(alphas),
                regs=tuple(regs),
                accepted=tuple(accepted),
                solve_ok=tuple(solve_ok),
        ...
        if g_norm <= residual_rtol * g0_norm + residual_atol:
            diag = altypes.NewtonSolveDiag(
                converged=True,
                iters=k + 1,
                g_norms=tuple(g_norms),
        ...
        if reject_streak >= max_rejects:
            diag = altypes.NewtonSolveDiag(
                ...
                accepted=tuple(accepted),
                solve_ok=tuple(solve_ok),
                reason="too_many_rejected_steps",
            )
            # print(f"DEBUG: Reject Streak Diagnosis: {diag}")
            return (op_last_accepted if return_last_accepted else op), diag
...
#     """
#     Newton-like inner solve that targets stationarity (ALGAMES-style optimality) rather
#     than full residual root finding.

#     The underlying step is computed from the packed residual system G(z)=0 (via autodiff),
#     but termination is based on the stationarity components only:
#         opt_vio_inf = max(abs(dLdX), abs(dLdU)) <= opt_tol

#     This mirrors ALGAMES.jl, where the inner loop breaks primarily when the optimality
#     violation drops below ϵ_opt, not when a global residual norm hits a tiny value.

#     Parameters
#     ----------
#     nlgame, op0, alstate : problem definition, initial primal-dual trajectory, and AL parameters.
#     discretize_method, ineq_activation : forwarded to residual builder.
#     opt_tol : float
#         Stationarity tolerance (ALGAMES ϵ_opt).
#     max_iters : int
...
#     if nlgame.tg != op0.tg:
#         raise ValueError(f"TimeGrid mismatch: nlgame.tg={nlgame.tg} vs op0.tg={op0.tg}")
#     if not isinstance(opt_tol, (float, int)) or not (float(opt_tol) > 0.0) or not math.isfinite(float(opt_tol)):
...
#     opt0 = optimality_violation_inf(
#         nlgame, op, alstate, discretize_method=discretize_method, ineq_activation=ineq_activation
#     )
...
#     dyn0 = float(jnp.max(jnp.abs(D))) if D.size else 0.0

    
#     # -- decision step size and residual (opt+dyn) merit metrics --
#     z0 = pack_decision_vars_1d(op)
#     g0 = compute_al_residual_flat_from_decision_vars(
#         nlgame, z0, op, alstate, discretize_method=discretize_method, ineq_activation=ineq_activation
#     )

#     if not bool(jnp.all(jnp.isfinite(g0))):
#         diag = altypes.StationarityNewtonDiag(
#             converged=False,
#             iters=0,
#             opt_vios=(float("nan"),),
#             merit_norms=(float("nan"),),
#             step_norms=tuple(),
#             alphas=tuple(),
#             regs=tuple(),
#             accepted=tuple(),
...
#     merit_norms.append(residual_norm(g0, kind=normkind))
...
#         # compute metrics after (possible) update
#         optk = optimality_violation_inf(
#             nlgame, op, alstate, discretize_method=discretize_method, ineq_activation=ineq_activation
#         )
#         # NOTE: this is also likely to be a misuse/inefficient use of dynamics residual.
#         # This should have already been computed elsewhere and could be passed on for constraint checking
#         D = systypes.residual_discrete_dynamics_trajectory(nlgame.cs, op, method=discretize_method)
#         dynk = float(jnp.max(jnp.abs(D))) if D.size else 0.0
#         zk = pack_decision_vars_1d(op)
#         gk = compute_al_residual_flat_from_decision_vars(
#             nlgame, zk, op, alstate, discretize_method=discretize_method, ineq_activation=ineq_activation
#         )
#         if not bool(jnp.all(jnp.isfinite(gk))):
#             diag = altypes.StationarityNewtonDiag(
#                 converged=False,
#                 iters=k + 1,
#                 opt_vios=tuple(opt_vios) + (float("nan"),),
#                 merit_norms=tuple(merit_norms) + (float("nan"),),
#                 step_norms=tuple(step_norms),
#                 alphas=tuple(alphas),
#                 regs=tuple(regs),
#                 accepted=tuple(accepted),
...
#         merit_norms.append(residual_norm(gk, kind=normkind))
...
#         # If newton_step_autodiff decided the direction is too small (alpha==0 but accepted),
#         # we are at a numerical/stall point.
#         if step_diag.accepted and step_diag.alpha == 0.0:
#             if optk <= opt_tol:
#                 reason = "opt_tol_and_step_stall"
#                 converged = True
#             else:
#                 reason = "step_stall_before_opt_tol"
#                 converged = False
...
#                 reason=reason,
#             )
#             return (op if converged else (op_last_accepted if return_last_accepted else op)), diag
...
def newton_solve_stationarity_autodiff(
    ...
    *,
    ...
    def _compute_metrics(cur_op: trajtypes.FixedStepPrimalDualTrajectory) -> Tuple[float, float, float, bool]:
        """
        Compute optimality/stationarity/feasibility metrics
        (opt_vio_inf, dyn_vio_inf, merit_norm, finite_ok) at the given op.
        """
        # -- optimality/stationarity metric --

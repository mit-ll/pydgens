...
def test_newton_solve_autodiff_rejects_invalid_params():
    nlgame, op0, alstate = _stub_game_op_alstate_for_solve()

    with pytest.raises(ValueError, match="max_iters"):
        pdg_alsolver.newton_solve_autodiff(
            nlgame, op0, alstate,
            max_iters=-1,
            step_rtol = 1e-7,
            step_atol = 1e-8,
...
def test_newton_solve_autodiff_rejects_timegrid_mismatch(monkeypatch):
    tg_game = TimeGrid(nt=4, dt=0.1, t0=0.0)
    tg_op = TimeGrid(nt=4, dt=0.1, t0=1.0)
    nlgame = SimpleNamespace(tg=tg_game)
    ...
    monkeypatch.setattr(pdg_alsolver, "pack_decision_vars_1d", lambda *_: (_ for _ in ()).throw(AssertionError("should not pack")))

    with pytest.raises(ValueError, match="TimeGrid mismatch"):
        pdg_alsolver.newton_solve_autodiff(nlgame, op0, alstate,
            step_rtol = 1e-7,
            step_atol = 1e-8,
            discretize_method = "rk2",
            ineq_activation = "altro",
...
def test_newton_solve_autodiff_returns_reason_on_nonfinite_residual_at_start(monkeypatch):
    nlgame, op0, alstate = _stub_game_op_alstate_for_solve()
    ...
    monkeypatch.setattr(
        pdg_alsolver,
        "compute_al_residual_flat_from_decision_vars",
        lambda *args, **kwargs: jnp.array([jnp.inf], dtype=jnp.float32),
    )

    op_out, diag = pdg_alsolver.newton_solve_autodiff(
        nlgame, op0, alstate,
        step_rtol = 1e-7,
        step_atol = 1e-8,
        discretize_method="euler",
...
def test_newton_solve_autodiff_converged_at_start_does_not_step(monkeypatch):
    tg = TimeGrid(nt=4, dt=0.1, t0=0.0)
    nlgame = SimpleNamespace(tg=tg)
    op0 = SimpleNamespace(tg=tg)
    alstate = SimpleNamespace()
    ...
    monkeypatch.setattr(
        pdg_alsolver,
        "compute_al_residual_flat_from_decision_vars",
        lambda *args, **kwargs: jnp.array([0.0], dtype=jnp.float32),
    )

    # Ensure step is never called
    monkeypatch.setattr(
        pdg_alsolver,
        "newton_step_autodiff",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("should not call newton_step_autodiff")),
    )

    op_out, diag = pdg_alsolver.newton_solve_autodiff(nlgame, op0, alstate, 
        max_iters=10,
        residual_rtol = 1e-7,
        residual_atol = 1e-8,
        step_rtol = 1e-7,
    ...
    assert diag.iters == 0
    assert diag.reason == "converged_at_start"
    assert diag.g_norms == (0.0,)

def test_newton_solve_autodiff_reject_streak_returns_last_accepted(monkeypatch):
    tg = TimeGrid(nt=4, dt=0.1, t0=0.0)
    nlgame = SimpleNamespace(tg=tg)
    op0 = SimpleNamespace(tg=tg, name="op0")
    alstate = SimpleNamespace()

    # residual norms from compute_al_residual_flat...:
    # - start: 10
    # - after accept: 5
    # - then rejects: stays 5 (since op doesn't change)
    def fake_pack(op):
        return op.name  # "z" is just the name

    def fake_G(nlgame_, z, op, alstate_, **kwargs):
        # decide based on which op is current
        if op.name == "op0":
    ...
    monkeypatch.setattr(pdg_alsolver, "pack_decision_vars_1d", fake_pack)
    monkeypatch.setattr(pdg_alsolver, "compute_al_residual_flat_from_decision_vars", fake_G)

    # Step schedule: accept once (opA), then reject forever
    calls = {"k": 0}
    def fake_step(nlgame_, op, alstate_, **kwargs):
        ...
        if calls["k"] == 1:
            ...
            diag = altypes.NewtonStepDiag(
                ...
                g_norm0=5.0, g_norm_trial=5.0,
                step_norm=1.0, reg=0.0,
                ls_iters=2, solve_ok=True
            )
            return op, diag  # unchanged
    ...
    assert op_out.name == "opA"  # last accepted, not op0
    assert diag.iters == 1 + 3  # one accept + 3 rejects to hit streak
    assert diag.accepted[0] is True
    assert all(a is False for a in diag.accepted[1:])


def test_newton_solve_autodiff_makes_progress_nonmock_float32():
    tg = TimeGrid(nt=4, dt=0.2, t0=0.0)
    nt = tg.nt
    K = nt - 1
    nx = 2
    ...
    # Basic sanity
    assert len(diag.g_norms) == diag.iters + 1
    assert diag.g_norms[0] > 0.0

    # Should make progress at least once
    assert min(diag.g_norms) < diag.g_norms[0]

    # Should have accepted at least one step unless it converged at start (it didn't)
    assert any(diag.accepted)

    # Should not terminate due to nonfinite residual
    assert "nonfinite" not in diag.reason
